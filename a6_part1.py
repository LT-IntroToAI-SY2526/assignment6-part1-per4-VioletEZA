...
def split_data(data):
    ...
    X = data[['Hours']]  # Features - note the double brackets to keep it as DataFrame
...
def visualize_results(X_train, y_train, X_test, y_test, predictions, model):
    ...
    plt.title('Linear Regression: Test Scores Predictions', fontsize=14, fontweight='bold')
...
def make_prediction(model, hours):
    ...
    print(f"\n=== New Prediction ===")
    print(f"If hours studied is {hours}hrs, predicted score: {predicted_scores:.2f}%")
